...
def load_pretrained_model(ratio, model_name):
    path = f"/media/lab/CH/cmfs/checkpoint-{model_name}-{ratio}-best.pth"
...
def cff4(ratio, pretrained=False, **kwargs):
    model = MSMF(ratio, block_size=32, dim=32, depth=2, reduction=8)
    if pretrained:
